...
namespace AIPrompt
{
    internal class AIPromptRootCommand : RootCommand
    {
        ...
        private static readonly Argument<string> argPrompt = new Argument<string>("prompt", "the prompt to be sent to the HTTP AI API");
        ...
        public static Parser Create(Action<AIPromptArguments> command, string? standardInput)
        {
            ...
            var parser = new CommandLineBuilder(rootCommand)
                ...
                .UseHelp(ctx =>
                {
                    ...
                    ctx.HelpBuilder.CustomizeSymbol(optInteractive, "-i", "interactive mode");
                })
        ...
        private static void SetHelpApperence(HelpContext ctx)
        {
            ctx.HelpBuilder.CustomizeLayout(_ => HelpBuilder.Default.GetLayout()
                ...
                .Append(_ => _.Output.WriteLine(@"  aiprompt.exe ""basic_prompt"" -h http://localhost:9511/api?command=chatgpt -u arguments.Username -p password -model AI -l"))
        ...
        private void ParseArguments(InvocationContext context){
            var arguments = new AIPromptArguments()
            {
                ...
                Model = context.ParseResult.GetValueForOption(optModel) ?? EnvironmentVariable.MODEL ?? "AI",
            ...
            if (arguments.Prompt == null && standardInput == null)
            {
